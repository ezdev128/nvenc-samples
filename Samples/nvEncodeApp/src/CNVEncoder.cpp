...
unsigned int CNvEncoder::GetCodecProfile(GUID encodeProfileGUID)
{
    ...
    else if (compareGUIDs(encodeProfileGUID, NV_ENC_H264_PROFILE_HIGH_444_GUID))
    {
        return 244;
    }
    else if(compareGUIDs(encodeProfileGUID, NV_ENC_H264_PROFILE_CONSTRAINED_HIGH_GUID ))
    {
        return 257;
    }
    else if (compareGUIDs(encodeProfileGUID, NV_ENC_CODEC_PROFILE_AUTOSELECT_GUID))
    {
        return 0;
    }
    else
...
GUID CNvEncoder::GetCodecProfileGuid(unsigned int profile)
{
    ...
    {
        // unknown profile
        printf("CNvEncoder::GetCodecProfile is an unspecified\n");
...
HRESULT CNvEncoder::InitD3D9(unsigned int deviceID)
{
    ...
    {
        printf("CNvEncoder::InitD3D() - deviceID=%d is outside range [%d,%d]\n", deviceID, 0, m_pD3D->GetAdapterCount());
        return E_FAIL;
    }
...
HRESULT CNvEncoder::InitD3D10(unsigned int deviceID)
{
    ...
    if(pFactory->EnumAdapters(deviceID, &pAdapter) != DXGI_ERROR_NOT_FOUND)
    {
           ...
           if(hr !=S_OK)
               printf("> Problem while creating %d D3d10 device \n",deviceID);
    }
    else
    {
        printf("> nvEncoder did not find %d D3D10 device \n",deviceID);
        return E_FAIL;
    }
...
HRESULT CNvEncoder::InitD3D11(unsigned int deviceID)
{
    ...
    if(pFactory->EnumAdapters(deviceID, &pAdapter) != DXGI_ERROR_NOT_FOUND)
    {
           ...
           if(hr !=S_OK)
               printf("> Problem while creating %d D3d11 device \n",deviceID);
    }
    else
    {
        printf("> nvEncoder did not find %d D3D11 device \n",deviceID);
        return E_FAIL;
    }
    return hr;
}
...
HRESULT CNvEncoder::InitCuda(unsigned int deviceID)
{
    ...
    {
        printf(">> InitCUDA() - cuInit() failed error:0x%x\n", cuResult);
        return E_FAIL;
    }
    ...
    if (deviceCount == 0)
    {
        printf(">> InitCuda() - reports no devices available that support CUDA\n");
        exit(EXIT_FAILURE);
    }
    else
    {
        printf(">> InitCUDA() has detected %d CUDA capable GPU device(s)<<\n", deviceCount);
        ...
        for (int currentDevice=0; currentDevice < deviceCount; currentDevice++)
        {
            ...
            printf("  [ GPU #%d - < %s > has Compute SM %d.%d, %s NVENC ]\n",
    ...
    {
        printf("  [ GPU %d does not have NVENC capabilities] exiting\n", deviceID);
        exit(EXIT_FAILURE);
    }
    ...
    for (unsigned int i = 0; i < m_dwMaxSurfCount; i++)
    {
        ...
        else
        {
            ...
            if (m_stEncoderInput[0].chromaFormatIDC == 3)
                stAllocInputSurface.bufferFmt = IsYUV444Tiled64x16Format(m_dwInputFormat) ? NV_ENC_BUFFER_FORMAT_YUV444_PL : m_dwInputFormat;
            else
...
HRESULT CNvEncoder::ReleaseEncoderResources()
{
    ...
    if (m_uRefCount==0)
    {
        printf("CNvEncoder::ReleaseEncoderResources() m_RefCount == 0, releasing resources\n");
        ReleaseIOBuffers();
        ...
        if (m_cuContext)
        {
            ...
            if (cuResult != CUDA_SUCCESS)
                printf("cuCtxDestroy error:0x%x\n", cuResult);
        }
...
HRESULT CNvEncoder::GetPresetConfig(int iPresetIdx)
{
    ...
    if(iPresetIdx == 7) // reserved preset value being passed by user
    {
        nvStatus = NV_ENC_ERR_INVALID_PARAM;
    }
    else
    {
...
void CNvEncoder::ParseCommandlineInput(int argc, const char *argv[], unsigned int *cmdArgFlags)
{

    //Command line overwriting the preset & configFile value
    if(getCmdLineArgumentValue(argc, (const char **)argv, "interfaceType",         &m_stEncoderInput[0].interfaceType))
    {
        cmdArgFlags[0] |= (1<<CMD_0_INTERFACE_TYPE);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "preset",                &m_stEncoderInput[0].preset))
    {
        cmdArgFlags[0] |= (1<<CMD_0_PRESET);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "chromaformatIDC",       &m_stEncoderInput[0].chromaFormatIDC))
    {
        cmdArgFlags[0] |= (1<<CMD_0_CHROMA_FORMAT_IDC);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "fieldMode",          &m_stEncoderInput[0].fieldEncoding))
    {
        cmdArgFlags[0] |= (1<<CMD_0_FIELD_MODE);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "bottomfieldFirst",      &m_stEncoderInput[0].bottomFieldFrist))
    {
        cmdArgFlags[0] |= (1<<CMD_0_BOTTOM_FIELD_FIRST);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "bitrate",               &m_stEncoderInput[0].avgBitRate))
    {
        cmdArgFlags[0] |= (1<<CMD_0_BITRATE);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "maxbitrate",            &m_stEncoderInput[0].peakBitRate))
    {
        cmdArgFlags[0] |= (1<<CMD_0_MAX_BITRATE);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "rcMode",                &m_stEncoderInput[0].rateControl))
    {
        cmdArgFlags[0] |= (1<<CMD_0_RC_MODE);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "numBFrames",            &m_stEncoderInput[0].numBFrames))
    {
        cmdArgFlags[0] |= (1<<CMD_0_NUM_B_FRAMES);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "frameRateNum",          &m_stEncoderInput[0].frameRateNum))
    {
        cmdArgFlags[0] |= (1<<CMD_0_FRAME_RATE_NUM);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "frameRateDen",          &m_stEncoderInput[0].frameRateDen))
    {
        cmdArgFlags[0] |= (1<<CMD_0_FRAME_RATE_DEN);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "gopLength",             &m_stEncoderInput[0].gopLength))
    {
        cmdArgFlags[0] |= (1<<CMD_0_GOP_LENGTH);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "enableInitialRCQP",     &m_stEncoderInput[0].enableInitialRCQP))
    {
        cmdArgFlags[0] |= (1<<CMD_0_ENABLE_INITIAL_RCQP);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "initialQPI",            &m_stEncoderInput[0].initialRCQP.qpIntra))
    {
        cmdArgFlags[0] |= (1<<CMD_0_INITIAL_QPI);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "initialQPP",            &m_stEncoderInput[0].initialRCQP.qpInterP))
    {
        cmdArgFlags[0] |= (1<<CMD_0_INITIAL_QPP);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "initialQPB",            &m_stEncoderInput[0].initialRCQP.qpInterB))
    {
        cmdArgFlags[0] |= (1<<CMD_0_INITIAL_QPB);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "profile",               &m_stEncoderInput[0].profile))
    {
        cmdArgFlags[0] |= (1<<CMD_0_PROFILE);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "level",                 &m_stEncoderInput[0].level))
    {
        cmdArgFlags[0] |= (1<<CMD_0_LEVEL);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "sliceMode",             &m_stEncoderInput[0].sliceMode))
    {
        cmdArgFlags[0] |= (1<<CMD_0_SLICE_MODE);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "sliceModeData",         &m_stEncoderInput[0].sliceModeData))
    {
        cmdArgFlags[0] |= (1<<CMD_0_SLICE_MODE_DATA);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "vbvBufferSize",         &m_stEncoderInput[0].vbvBufferSize))
    {
        cmdArgFlags[0] |= (1<<CMD_0_VBV_BUFFERSIZE);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "vbvInitialDelay",       &m_stEncoderInput[0].vbvInitialDelay))
    {
        cmdArgFlags[0] |= (1<<CMD_0_VBV_INITIAL_DELAY);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "enablePtd",             &m_stEncoderInput[0].enablePTD))
    {
        cmdArgFlags[0] |= (1<<CMD_0_ENABLE_PTD);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "separateColourPlaneFlag", &m_stEncoderInput[0].separateColourPlaneFlag))
    {
        cmdArgFlags[0] |= (1<<CMD_0_SEPARATE_COLOUR_PLANE_FLAG);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "enableAQ",               &m_stEncoderInput[0].enableAQ))
    {
        cmdArgFlags[0] |= (1<<CMD_0_ENABLE_AQ);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "intraRefreshCount",      &m_stEncoderInput[0].intraRefreshCount))
    {
        cmdArgFlags[0] |= (1<<CMD_0_INTRA_REFRESH_COUNT);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "intraRefreshPeriod",     &m_stEncoderInput[0].intraRefreshPeriod))
    {
        cmdArgFlags[0] |= (1<<CMD_0_INTRA_REFRESH_PERIOD);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "idrperiod",              &m_stEncoderInput[0].idr_period))
    {
        cmdArgFlags[0] |= (1<<CMD_0_IDR_PERIOD);
    }

    char *InputFile =NULL;
    if(getCmdLineArgumentString(argc, (const char **)argv, "inFile"            ,&InputFile))
    {
        cmdArgFlags[0] |= (1<<CMD_0_INPUT_FILE);
    }
    if( InputFile != NULL)
        memcpy(&m_stEncoderInput[0].InputClip, InputFile, 256);

    if(getCmdLineArgumentValue(argc, (const char **)argv, "syncMode",              &m_stEncoderInput[0].syncMode))
    {
        cmdArgFlags[0] |= (1<<CMD_0_SYNC_MODE);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "width",                 &m_stEncoderInput[0].width))
    {
        cmdArgFlags[0] |= (1<<CMD_0_WIDTH);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "height",                &m_stEncoderInput[0].height))
    {
        cmdArgFlags[0] |= (1<<CMD_0_HEIGHT);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "maxwidth",              &m_stEncoderInput[0].maxWidth))
    {
        cmdArgFlags[1] |= (1<<CMD_1_MAXWIDTH);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "maxheight",             &m_stEncoderInput[0].maxHeight))
    {
        cmdArgFlags[1] |= (1<<CMD_1_MAXHEIGHT);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "cabacenable",           &m_stEncoderInput[0].vle_cabac_enable))
    {
        cmdArgFlags[1] |= (1<<CMD_1_CABAC_ENABLE);
    }
    if(getCmdLineArgumentValue(argc, (const char **)argv, "mvprecision",           &m_stEncoderInput[0].mvPrecision))
    {
        cmdArgFlags[1] |= (1<<CMD_1_MV_PRECISION);
    }
...
HRESULT CNvEncoder::OpenEncodeSession(int argc, const char *argv[],unsigned int deviceID)
{
    ...
    else
    {
        ...
        if (nvStatus != NV_ENC_SUCCESS)
        {
            printf("File: %s, Line: %d, nvEncGetEncodeGUIDCount() returned with error %d\n", __FILE__, __LINE__, nvStatus);
            checkNVENCErrors(nvStatus);
        }
        else
        {
            ...
            if (nvStatus != NV_ENC_SUCCESS)
            {
                printf("File: %s, Line: %d, nvEncGetEncodeGUIDs() returned with error %d\n", __FILE__, __LINE__, nvStatus);
                checkNVENCErrors(nvStatus);
            }
            else
            {
        ...
        if (nvStatus != NV_ENC_SUCCESS)
        {
            printf("File: %s, Line: %d, nvEncGetEncodeProfileGUIDCount() returned with error %d\n", __FILE__, __LINE__, nvStatus);
            checkNVENCErrors(nvStatus);
        }
        else
        {
            ...
            if (nvStatus != NV_ENC_SUCCESS)
            {
                printf("File: %s, Line: %d, nvEncGetEncodeProfileGUIDs() returned with error %d\n", __FILE__, __LINE__, nvStatus);
                checkNVENCErrors(nvStatus);
            }
            else
            {
        ...
        if (nvStatus != NV_ENC_SUCCESS)
        {
            printf("File: %s, Line: %d, nvEncGetInputFormatCount() returned with error %d\n", __FILE__, __LINE__, nvStatus);
            checkNVENCErrors(nvStatus);
        }
        else
        {
            ...
            if (nvStatus != NV_ENC_SUCCESS)
            {
                printf("File: %s, Line: %d, nvEncGetInputFormats() returned with error %d\n", __FILE__, __LINE__, nvStatus);
                checkNVENCErrors(nvStatus);
            }
            else
            {
                ...
                for (idx = 0; idx < m_dwInputFmtCount; idx++)
                {
                    if(m_stEncoderInput[0].chromaFormatIDC == 3)
                    {
                        if (m_pAvailableSurfaceFmts[idx] == NV_ENC_BUFFER_FORMAT_YUV444_TILED64x16)
                        {
                            bFmtFound = true;
                            break;
                        }
